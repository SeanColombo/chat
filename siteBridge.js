 ...
 * @date: 20150728
 *
 * The SiteBridge allows interfacing with the main website that this server is related to.
 *
 * This allows the normal site's authentication to be used when accessing this server, so
 * that users have a seamless experience and can easily understand the site and the server
 * to be part of the same system (they don't need to know that they run on different
 * technology & they never have to re-login, etc.).
 *
 * NOTE: CURRENTLY REFACTORING THIS TO BE LESS MediaWiki SPECIFIC. For MediaWiki installs,
 * just use the MWBridge file instead.
 */
...
// This will be logger and monitor's parent, so we need to copy the config from this script's parent.
...
SiteBridge.prototype.authenticateUser = function(roomId, name, key, handshake, success, error) {
...
SiteBridge.prototype.ban = function(client, name, time, reason, success, error) {
...
SiteBridge.prototype.giveChatMod = function(roomId, name, handshake, key, success, error) {
	clearAuthenticateCache(roomId, name);
...
// TODO: Disambiguate this... it's using the same function name as the func above... rename that one to _Internal or something (I think the call below in the setInterval() loop is to the internal one).
SiteBridge.prototype.setUsersList = function(roomId, users) {
...
exports.SiteBridge = new SiteBridge();
